...
export function TogglePublicPrivate({
  ...
  const handleToggle = async () => {
    ...
    try {
      const endpoint = entityType === "catalog"
        ? `/api/catalogs/${entityId}/update`
        : `/api/archives/${entityId}/update`;
      ...
      if (response.ok) {
        ...
        if (data.success) {
          startTransition(() => {
            ...
            setRateLimitRemaining(null);
            toast.success(
              `${entityType === "catalog" ? "Catalog" : "Archive"} is now ${newIsPublic ? "public" : "private"}`
            );
            // Refresh the page to update the data from server
            router.refresh();
          });
        } else {
          toast.error(data.message || `Failed to update ${entityType} visibility`);
        }
      } else {
        // Handle non-OK response
        let retryAfterSeconds: number | null = null;
        let errorMessage = `Failed to update ${entityType} visibility`;
        ...
        if (data?.error?.code === "RATE_LIMIT_EXCEEDED" || response.status === 429) {
          ...
          errorMessage = `${entityType === "catalog" ? "Catalog" : "Archive"} not found`;
        } else if (response.status === 403) {
          errorMessage = `You don't have permission to update this ${entityType}`;
      ...
      console.error(`Error toggling ${entityType} visibility:`, error);
